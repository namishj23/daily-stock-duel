...
// POST - Calculate results for a specific contest date
export async function POST(request: NextRequest) {
    try {
        ...
        for (const prediction of predictions) {
            ...
            // Calculate prediction accuracy (how close to actual percentage)
            const predictionError = Math.abs(actualChange - prediction.predictedChange)
            ...
            updates.push(
                prisma.dailyPrediction.update({
                    ...
                    data: {
                        ...
                        // We'll determine winner separately
                    },
                })
            )
        }
        ...
        if (updatedPredictions.length === 0) {
            return NextResponse.json({
                message: 'Stock prices updated but no complete predictions',
